...
if( file_name == 'Exit_preds.csv'):   
    higher_dataset = dataset[(dataset.Probabilities >= 0.75)]   
    lower_dataset = dataset[(dataset.Probabilities < 0.75)]

else:
    higher_dataset = dataset[(dataset.Probabilities >= 0.25)]   
    lower_dataset = dataset[(dataset.Probabilities < 0.25)]


if segment == 0:
    X = lower_dataset.iloc[:, [4, 7, 8, 9, 10, 11, 12, 13]].values 
    fifties = lower_dataset[(dataset.Age >= 50)]
else:
    X = higher_dataset.iloc[:, [4, 7, 8, 9, 10, 11, 12, 13]].values
    fifties = higher_dataset[(dataset.Age >= 50)]
    


#lower_dataset = dataset[(dataset.Probabilities < 0.75)]
#X = lower_dataset.iloc[:, [6, 7, 8, 9, 10, 11, 12, 13]].values
...
plt.scatter(X[ykmeans == 2, 0], X[ykmeans == 2, 1], marker = '*', s = 30, c = 'magenta', label = 'Target')
#plt.scatter(X[ykmeans == 3, 0], X[ykmeans == 3, 1], marker = '*', s = 30, c = 'yellow', label = 'Careful')
#plt.scatter(X[ykmeans == 4, 0], X[ykmeans == 4, 1], s = 100, c = 'blue', label = 'Sensible')
#plt.scatter(X[ykmeans == 5, 0], X[ykmeans == 5, 1], s = 100, c = 'green', label = 'Sensless')
#plt.scatter(X[ykmeans == 6, 0], X[ykmeans == 6, 1], s = 100, c = 'gray', label = 'Sense')
#plt.scatter(X[ykmeans == 7, 0], X[ykmeans == 6, 1], s = 100, c = 'violet', label = 'Sensless')
...
plt.title('Clusters of Clients with exit probability between 0.25 and 0.5')
...
plt.show()
plt.draw()
fig1.savefig('Exit probability between 0.25 and 0.5.png', dpi=300)
...
plt.title('Comparing people above 50 and those below for the diff probabilites')
plt.xlabel('Exit probability')
plt.ylabel('Pecentage of people(%)')
plt.legend()
fig1 = plt.gcf()
plt.show()
plt.draw()
